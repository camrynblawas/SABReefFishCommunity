...
#Definitely some changes in sampling effort over time. Let's only use resampled stations and do just after 1995. 

resampled <- fishdf  %>%
  group_by(station) %>%
  filter(all(1995:2019 %in% year)) %>% filter(year > 1994)
...
fishdfpost2010 <- fishdfpost2010 %>% dplyr::select(eventname, date, year, month, eventname, family, speciesscientificname, speciescommonname, numbertotal, speciestotalweight, tempsurface, tempbottom, salinitysurface, salinitybottom, sdo, bdo, latitudestart, longitudestart, depthstart, duration)

...
for (i in 1:length(top10names)) {
  ...
  data <- filter(fishdf, accepted_name == top10names[i])
...
ggplot(finalvips2, mapping = aes(x = Variable)) + geom_bar() + my_theme() + labs(title = "xgboost Highest Variable Importance by Family", x = "Variable", y = "Number of Families with Variable as Important")
ggsave(here("figures", "CountofVariableImportance.png"), width = 6, height = 4, unit = "in")
```

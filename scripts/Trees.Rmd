...
```
xgboost performs a little better. Interesting! Let's pick out a the top 10 species and figure out variable importance 

#Multiple species
```{r}
top10names <- top10names[-6] 
#Grey triggerfish was giving me trouble
rmse <- list()
vips <- list()
for (i in 1:length(top10names)) {
  label <- str_to_title(top10names[i])
  data <- filter(fishdf, speciesscientificname == top10names[i])
  data <- data %>% dplyr::select(year, month, tempsurface, tempbottom, salinitybottom, salinitysurface, sdo, bdo, latitudestart, longitudestart, depthstart, weight, sexdescription, maturitydescription)
 ...
 vips[[i]] <- vip
  ...
  p <- res_boost %>%
    ...
    .[['fit']] %>%
    vip() +
    theme_minimal() + labs(title = paste0(label, " Random Forest Variable Importances"), x = "Variable", y = "Importance")
  print(p)
}

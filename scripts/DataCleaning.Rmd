...
for(i in 8:nrow(uniquescinames)) {
...
event <- read.csv(here("data", "rawdata", "cblawas.Reef Fish Survey.EVENT.2023-08-31T17.00.10.csv"))
...
new <- merge(df, event, by = "eventname")

#export data with depths to processed
# write.csv(new, here("data", "processeddata", "RFS_AbundanceBiomassDepth.csv"))
```

```{r}
#check out length data
length <- read.csv(here("data", "rawdata", "cblawas.Reef Fish Survey.LENGTHFREQUENCY.2023-09-04T22.21.03.csv"))
...
specimen <- read.csv(here("data", "rawdata", "cblawas.Reef Fish Survey.SPECIMEN.2023-09-04T22.18.50.csv"))
...
```{r}
#sampling effort EDA
ggplot(data = df, mapping = aes(x = year, y = month)) + geom_point()
...
stations <- read_csv(here("data", "rawdata", "stationssampled.csv"))
stations_longer <- stations %>% pivot_longer(cols = !StationNumber, names_to = "year", values_to = "count")
stations_longer <- filter(stations_longer, year != "Total")
stations_longer$year <- as.numeric(stations_longer$year)
stations_longer <- mutate(stations_longer, decade = floor_decade(year))
stations_longer <- stations_longer %>% group_by(StationNumber, decade) %>% summarise(count = sum(count, na.rm = TRUE))
stations_sampled <- filter(stations_longer, count != 0)
stations_wider <- pivot_wider(stations_sampled, names_from = decade, values_from = count)
stations_wider <- na.omit(stations_wider)
stationsloc <- read_csv(here("data", "rawdata", "possibleStations.csv"))
finalstations <- merge(stations_wider, stationsloc, by = "StationNumber")
write.csv(finalstations, here("data", "processeddata", "resampledstations.csv"))
